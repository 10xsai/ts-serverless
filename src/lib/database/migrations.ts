...
export class MigrationManager {
     ...
     * @returns Migration manager instance
     */
    public registerAll(migrations: Migration[]): this {
    ...
    public async getAppliedMigrations(): Promise<MigrationStatus[]> {
        try {
            ...
            const results = await this.adapter.query<MigrationStatus>(
                'SELECT * FROM _migrations ORDER BY appliedAt ASC'
            );
            return results;
        } catch (error) {
     ...
     */
    public async executeMigration(migration: Migration): Promise<MigrationStatus> {
        ...
        try {
            ...
            await this.adapter.transaction(async (tx) => {
                ...
                await tx.execute(
                    'INSERT INTO _migrations (id, appliedAt, executionTime, success) VALUES (?, ?, ?, ?)',
                    [migration.id, new Date(), Date.now() - startTime, true]
                );
            });
            ...
            try {
                await this.adapter.execute(
                    'INSERT INTO _migrations (id, appliedAt, executionTime, success, error) VALUES (?, ?, ?, ?, ?)',
                    [migration.id, status.appliedAt, status.executionTime, false, status.error]
                );
            } catch {
     ...
     */
    public async rollbackMigration(migration: Migration): Promise<MigrationStatus> {
        const startTime = Date.now();
...
export class MigrationBuilder {
     ...
     * @returns Migration builder instance
     */
    public addUp(statement: string): this {
     ...
     * @returns Migration builder instance
     */
    public addDown(statement: string): this {
 ...
 * @returns Migration manager instance
 */
export function createMigrationManager(adapter: DatabaseAdapter): MigrationManager {
